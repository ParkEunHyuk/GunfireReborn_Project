#include "stdafx.h"
#include "..\Public\Monster_Catfish.h"
#include "GameInstance.h"
#include "Item_Universal.h"


CMonster_Catfish::CMonster_Catfish(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CLandObject(pDevice, pDeviceContext)
{

}

CMonster_Catfish::CMonster_Catfish(const CMonster_Catfish & rhs)
	: CLandObject(rhs)
{
}

HRESULT CMonster_Catfish::NativeConstruct_Prototype()
{
...
HRESULT CMonster_Catfish::NativeConstruct(void * pArg)
{
...
_int CMonster_Catfish::Tick(_double TimeDelta)
{
	...
	{
		MSGBOX("CMonster_Catfish의 Add_ObseverGroup에 문제가 발생했습니다");
		return E_FAIL;
	}
...
_int CMonster_Catfish::LateTick(_double TimeDelta)
{
...
HRESULT CMonster_Catfish::Render()
{
...
HRESULT CMonster_Catfish::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(m_eNowLevelNum, TEXT("Prototype_Component_Model_Catfish"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
HRESULT CMonster_Catfish::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CMonster_Catfish::SetUp_Event_Tirgger(_double TimeDelta)
{
	return S_OK;
}

HRESULT CMonster_Catfish::SetUp_Fight(_double TimeDelta)
{
	Fight_On();
...
HRESULT CMonster_Catfish::Fight_On()
{
...
HRESULT CMonster_Catfish::Once_Trigger()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CMonster_Catfish::PlayAnim(_double TimeDelta)
{
...
HRESULT CMonster_Catfish::CoolTime_Manager(_double TimeDelta)
{
	m_dOnceCoolTime += TimeDelta;

	if (m_fDistance < 4 && m_dOnceCoolTime > 1)
	{
	...
	if (m_dOnceCoolTime > 7)
	{
		m_iOncePattern = rand() % 5;
		m_bOneTimeSwith = true;
		m_dOnceCoolTime = 0;
		m_dInfinityCoolTime = 0;
...
HRESULT CMonster_Catfish::Once_AnimMotion(_double TimeDelta) //무빙넘버와 공격넘버로 바꿔도 괜찮을듯?
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	...
	case 3: //왼쪽으로 구르기
		...
		break;
	case 4: //뒤로 점프
		m_iOnceAnimNumber = 13;
		m_bOneTimeSwith = true;
		if (m_dFireTime < 1)
		{
			m_dFireTime += TimeDelta;
			m_pTransform->Go_Backward(TimeDelta*1.5, m_pNavigationCom);
		}
		break;
	case 14:
		m_iOnceAnimNumber = 14; //뛰어들어가서 휘두름
		m_bOneTimeSwith = true;
		if (m_dFireTime < 0.8)
		{
			m_dFireTime += TimeDelta;
			...
			m_dFireTime += TimeDelta;
			Universal_RunAndAttack(TimeDelta);
			m_pTrail->Set_NotRendering(false);
		}
		break;
	case 15:
		m_iOnceAnimNumber = 15; // 평범하게 휘두름
		m_bOneTimeSwith = true;
...
HRESULT CMonster_Catfish::Infinity_AnimMotion(_double TimeDelta)
{
...
_int CMonster_Catfish::Obsever_Hit(void* pArg)
{
...
_int CMonster_Catfish::Obsever_Event(void * pArg)
{
	CUniversal_Bullet* SorceBullet = static_cast<CUniversal_Bullet*>(pArg);
...
_int CMonster_Catfish::Dead_Trigger()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CMonster_Catfish::Universal_Wield(_double TimeDelta)
{
	...
	if (true == m_bFire && m_dFireTime > 0.7)
	{
		...
		Universal_BulletDesc.vScale = _float3(9.f, 9.f, 9.f);
		...
		GetSingle(CGameInstance)->PlaySound((L"CatPish_Wield.mp3"), CHANNEL_EFFECT);
	}
...
HRESULT CMonster_Catfish::Universal_RunAndAttack(_double TimeDelta)
{
	...
	if (true == m_bFire && m_dFireTime > 1)
	{
		...
		GetSingle(CGameInstance)->PlaySound((L"Catfish_RunAndAttack.mp3"), CHANNEL_EFFECT);
	}
...
HRESULT CMonster_Catfish::Slow_On()
{
...
HRESULT CMonster_Catfish::Notice()
{
...
HRESULT CMonster_Catfish::SetUp_Trail()
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
	...
	TrailDesc.eMonsterType = CTrail::CATFISH;

	TrailDesc.vHighPos = _float3(-0.15f, -0.1f, 1.6f);
	TrailDesc.vLowPos = _float3(0.0f, 0.0f, 0.3f);

	TrailDesc.vColor = _float4(100.f / 255.f, 255.f / 255.f, 100.f / 255.f, 1.f);
...
void CMonster_Catfish::Set_Trail(CTrail * pTrail)
{
	m_pTrail = pTrail;
}

CMonster_Catfish * CMonster_Catfish::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CMonster_Catfish*	pInstance = new CMonster_Catfish(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CMonster_Catfish");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CMonster_Catfish::Clone(void * pArg)
{
	CMonster_Catfish*	pInstance = new CMonster_Catfish(*this);
