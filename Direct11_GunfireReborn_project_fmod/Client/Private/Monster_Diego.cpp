#include "stdafx.h"
#include "..\Public\Monster_Diego.h"
#include "GameInstance.h"
#include "Item_Universal.h"


CMonster_Diego::CMonster_Diego(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CLandObject(pDevice, pDeviceContext)
{

}

CMonster_Diego::CMonster_Diego(const CMonster_Diego & rhs)
	: CLandObject(rhs)
{
}

HRESULT CMonster_Diego::NativeConstruct_Prototype()
{
...
HRESULT CMonster_Diego::NativeConstruct(void * pArg)
{
...
_int CMonster_Diego::Tick(_double TimeDelta)
{
	...
	{
		MSGBOX("CMonster_Diego의 Add_ObseverGroup에 문제가 발생했습니다");
		return E_FAIL;
	}
...
_int CMonster_Diego::LateTick(_double TimeDelta)
{
...
HRESULT CMonster_Diego::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(m_eNowLevelNum, TEXT("Prototype_Component_Model_Diego"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
	...
	ColliderDesc.vScale = _float3(1.5f, 1.5f, 1.f);
	ColliderDesc.vRotation = _float4(0.f, 0.f, 0.f, 1.f);
	ColliderDesc.vTranslation = _float4(0.f, 0.5f, 0.f, 1.f);
...
HRESULT CMonster_Diego::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CMonster_Diego::SetUp_Event_Tirgger(_double TimeDelta)
{
	return S_OK;
}

HRESULT CMonster_Diego::SetUp_Fight(_double TimeDelta)
{
	Fight_On();
	...
	if (true == m_bFight)
	{
		...
		if (m_fDistance < 2)
		{
			m_pTransform->Go_Backward(TimeDelta*2.5, m_pNavigationCom);
		}
...
HRESULT CMonster_Diego::Fight_On()
{
...
HRESULT CMonster_Diego::Once_Trigger()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CMonster_Diego::PlayAnim(_double TimeDelta)
{
...
HRESULT CMonster_Diego::CoolTime_Manager(_double TimeDelta)
{
	m_dOnceCoolTime += TimeDelta;


	if (m_fDistance < 4 && m_dOnceCoolTime > 3)
	{
	...
	if (m_dInfinityCoolTime > 1)
	{
		m_iInfinityPattern = rand() % 9;
		m_dInfinityCoolTime = 0;
...
HRESULT CMonster_Diego::Once_AnimMotion(_double TimeDelta) //무빙넘버와 공격넘버로 바꿔도 괜찮을듯?
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	...
	case 0:
	{
		m_iOnceAnimNumber = 16; //왼쪽으로 점프
		m_bOneTimeSwith = true;

		m_dFireTime += TimeDelta;
		if (m_dFireTime>0.3 && m_dFireTime < 0.8)
		{
			m_pTransform->Go_Left(TimeDelta*3.0, m_pNavigationCom);
		}
		break;
	}
	case 1:
	{
		m_iOnceAnimNumber = 17; //오른쪽으로 점프
		m_bOneTimeSwith = true;

		m_dFireTime += TimeDelta;
		if (m_dFireTime>0.3 && m_dFireTime < 0.8)
		{
			m_pTransform->Go_Right(TimeDelta*3.0, m_pNavigationCom);
		}
		break;
	}
	case 12:
	{
		m_iOnceAnimNumber = 12; //몸통 박치기
		m_bOneTimeSwith = true;
	...
	case 13:
	{
		m_iOnceAnimNumber = 13; //연속 공격
		m_bOneTimeSwith = true;
		m_dFireTime += TimeDelta;
		if (m_dFireTime > 1.2 && 0 == m_iComboCount && true == m_bFire)
		{
			Universal_ComboAttack(TimeDelta);
			m_iComboCount += 1;
		}else if (m_dFireTime > 1.5 && 1 == m_iComboCount && true == m_bFire)
		{
			Universal_ComboAttack(TimeDelta);
			m_iComboCount += 1;
		}
		else if (m_dFireTime > 2.2 && 2 == m_iComboCount && true == m_bFire)
		{
			Universal_ComboAttack(TimeDelta);
	...
	case 15:
	{
		m_iOnceAnimNumber = 15; //물어 뜯기
		m_bOneTimeSwith = true;
		Universal_Biting(TimeDelta);
		break;
	}
	default:
		break;
...
HRESULT CMonster_Diego::Infinity_AnimMotion(_double TimeDelta)
{
	if (true == m_bFight)
	{
		...
		case 0: //앞으로 뛰어옴
			...
			m_pTransform->Go_Straight(TimeDelta*2.5, m_pNavigationCom);
			break;
		case 1: //뒤로 움직임
...
_int CMonster_Diego::Obsever_Hit(void* pArg)
{
...
_int CMonster_Diego::Obsever_Event(void * pArg)
{
	CUniversal_Bullet* SorceBullet = static_cast<CUniversal_Bullet*>(pArg);
...
_int CMonster_Diego::Dead_Trigger()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CMonster_Diego::Universal_Wield(_double TimeDelta)
{
	...
	if (true == m_bFire && m_dFireTime > 0.8)
	{
		...
		Universal_BulletDesc.fPositioning = _float3(0, 1.5, 0.5f);
		...
		GetSingle(CGameInstance)->PlaySound((L"Diego_Wield.mp3"), CHANNEL_EFFECT);
	}
...
HRESULT CMonster_Diego::Universal_Biting(_double TimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	...
	if (true == m_bFire && m_dFireTime > 0.8)
	{
		...
		GetSingle(CGameInstance)->PlaySound((L"Diego_Biting.mp3"), CHANNEL_EFFECT);
	}
...
HRESULT CMonster_Diego::Universal_ComboAttack(_double TimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	...
	if (true == m_bFire )
	{
		...
		GetSingle(CGameInstance)->PlaySound((L"Diego_ComboAttack.mp3"), CHANNEL_EFFECT);
...
HRESULT CMonster_Diego::Slow_On()
{
...
HRESULT CMonster_Diego::Notice()
{
	...
	Universal_BulletDesc.vScale = _float3(20.f, 20.f, 20.f);
	Universal_BulletDesc.vRotation = _float4(0.f, 0.f, 0.f, 1.f);
...
CMonster_Diego * CMonster_Diego::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CMonster_Diego*	pInstance = new CMonster_Diego(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CMonster_Diego");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CMonster_Diego::Clone(void * pArg)
{
	CMonster_Diego*	pInstance = new CMonster_Diego(*this);
