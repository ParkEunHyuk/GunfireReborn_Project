#include "stdafx.h"
#include "..\Public\Monster_Raven.h"
#include "GameInstance.h"
#include "Item_Universal.h"
#include "Monster_Lazer.h"


CMonster_Raven::CMonster_Raven(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CLandObject(pDevice, pDeviceContext)
{

}

CMonster_Raven::CMonster_Raven(const CMonster_Raven & rhs)
	: CLandObject(rhs)
{
}

HRESULT CMonster_Raven::NativeConstruct_Prototype()
{
...
HRESULT CMonster_Raven::NativeConstruct(void * pArg)
{
...
_int CMonster_Raven::Tick(_double TimeDelta)
{
	...
	{
		MSGBOX("Monster_Raven의 Add_ObseverGroup에 문제가 발생했습니다");
		return E_FAIL;
	}
...
_int CMonster_Raven::LateTick(_double TimeDelta)
{
...
HRESULT CMonster_Raven::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(m_eNowLevelNum, TEXT("Prototype_Component_Model_Raven"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
	...
	ColliderDesc.vScale = _float3(1.f, 1.5f, 1.5f);
	ColliderDesc.vRotation = _float4(0.f, 0.f, 0.f, 1.f);
...
HRESULT CMonster_Raven::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CMonster_Raven::SetUp_Event_Tirgger(_double TimeDelta)
{
	return S_OK;
}

HRESULT CMonster_Raven::SetUp_Fight(_double TimeDelta)
{
	Fight_On();
...
HRESULT CMonster_Raven::Fight_On()
{
...
HRESULT CMonster_Raven::Once_Trigger()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CMonster_Raven::PlayAnim(_double TimeDelta)
{
	...
	if (m_dAimTime < 1.5)
	{
		...
		m_pModelCom->Update_CombinedTransformationMatrices(0, m_bOneTimeSwith);
	}

	return S_OK;
}

HRESULT CMonster_Raven::CoolTime_Manager(_double TimeDelta)
{
	m_dOnceCoolTime += TimeDelta;


	if (m_fDistance < 4 && m_dOnceCoolTime > 4 )
	{
		m_iOncePattern = 11;
	...
	if (m_dOnceCoolTime > 4)
	{
		...
		else if (m_iOncePattern >= 2 ) //지금 현재 2이상은 전부 조준하는 모션이므로 이렇게 코드를 작성함 나중에 조심하자
		{
			m_bOneTimeSwith = true;
			m_dOnceCoolTime = 0;
			m_dInfinityCoolTime = 0;
...
HRESULT CMonster_Raven::Once_AnimMotion(_double TimeDelta) //무빙넘버와 공격넘버로 바꿔도 괜찮을듯?
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	...
	case 0:
	{
		m_iOnceAnimNumber = 8; //왼쪽으로 점프
		m_bOneTimeSwith = true;
		m_pTransform->Go_Left(TimeDelta *1.5, m_pNavigationCom);
		break;
	}
	case 1:
	{
		m_iOnceAnimNumber = 9; //오른쪽으로 점프
		m_bOneTimeSwith = true;
		m_pTransform->Go_Right(TimeDelta *1.5, m_pNavigationCom);
		break;
	}
	case 2:
	{
		m_iOnceAnimNumber = 2; //총 발사
		m_bOneTimeSwith = true;
...
HRESULT CMonster_Raven::Infinity_AnimMotion(_double TimeDelta)
{
	if (true == m_bFight)
	{
		...
		{
		case 0: //앞으로 움직임
			m_iInfinityAnimNumber = 12;
			m_pTransform->Go_Straight(TimeDelta*0.5, m_pNavigationCom);
			break;
		case 1: //뒤로 움직임
			m_iInfinityAnimNumber = 14;
			m_pTransform->Go_Backward(TimeDelta*0.2, m_pNavigationCom);
			break;
		case 2: //왼쪽으로 움직임
			m_iInfinityAnimNumber = 15;
			m_pTransform->Go_Left(TimeDelta*0.5, m_pNavigationCom);
			break;
		case 3: //오른쪽으로 움직임
			m_iInfinityAnimNumber = 13;
			m_pTransform->Go_Right(TimeDelta*0.5, m_pNavigationCom);
			break;
		case 4: //앞으로 움직임
			m_iInfinityAnimNumber = 12;
			m_pTransform->Go_Straight(TimeDelta*0.5, m_pNavigationCom);
		...
		case 5: //뒤로 움직임
			m_iInfinityAnimNumber = 14;
			m_pTransform->Go_Straight(TimeDelta*0.2, m_pNavigationCom);
			break;
		case 6: //앞으로 뛰어옴
...
_int CMonster_Raven::Obsever_Hit(void* pArg)
{
...
_int CMonster_Raven::Obsever_Event(void * pArg)
{
	CUniversal_Bullet* SorceBullet = static_cast<CUniversal_Bullet*>(pArg);
...
_int CMonster_Raven::Dead_Trigger()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CMonster_Raven::Universal_Bullet(_double TimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);

	if (true == m_bAimingOn)
	{
		m_dAimTime += TimeDelta;
		m_dInfinityCoolTime = 0;
		m_dOnceCoolTime = 0;

		if (false == m_bLazerSwitch && m_dFireTime>1.0)
		{
			SetUp_Laser(TimeDelta);
			m_bLazerSwitch = true;
		}
	}

	m_dFireTime += TimeDelta;
	if (true == m_bFire && m_dFireTime>5.0)
	{
		...
		Universal_BulletDesc.iBulletNumber = CUniversal_Bullet::BULLETTYPE::RAVEN;
		Universal_BulletDesc.eType = BULLET_MOSTER;

		Universal_BulletDesc.fSpeedPerSec = 80.f;
		Universal_BulletDesc.vScale = _float3(1.2f, 1.2f, 1.2f);
		Universal_BulletDesc.vRotation = _float4(0.f, 0.f, 0.f, 1.f);
		Universal_BulletDesc.dDuration = 10;

		Universal_BulletDesc.pObject = this;
		Universal_BulletDesc.fPositioning = _float3(0.2f, 0.8f, 0.f);
		...
		GetSingle(CGameInstance)->PlaySound((L"Raven_Shot.mp3"), CHANNEL_EFFECT);
	}
...
HRESULT CMonster_Raven::SetUp_Laser(_double TimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	...
	CMonster_Lazer::LAZERDESC			LazerDesc;
	LazerDesc.pModel = pModel;
	LazerDesc.pSocketName = "Gun_02_Nub";
	LazerDesc.pTransform = pTransform;
	LazerDesc.pObject = this;

	if (FAILED(pGameInstance->Add_GameObject(m_eNowLevelNum, TEXT("Layer_Monster_Razer"), TEXT("Prototype_GameObject_Monster_Lazer"), &LazerDesc)))
		return E_FAIL;
...
HRESULT CMonster_Raven::Universal_Wield(_double TimeDelta)
{
	...
	if (true == m_bFire && m_dFireTime > 0.4)
	{
		...
		GetSingle(CGameInstance)->PlaySound((L"Raven_Wield.mp3"), CHANNEL_EFFECT);
	}
...
HRESULT CMonster_Raven::Slow_On()
{
...
CMonster_Raven * CMonster_Raven::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CMonster_Raven*	pInstance = new CMonster_Raven(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CMonster_Raven");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CMonster_Raven::Clone(void * pArg)
{
	CMonster_Raven*	pInstance = new CMonster_Raven(*this);
