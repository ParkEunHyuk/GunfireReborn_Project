...
_int CMonster_Lamp::Tick(_double TimeDelta)
{
	...
	{
		MSGBOX("CMonster_Lamp의 Add_ObseverGroup에 문제가 발생했습니다");
		return E_FAIL;
	}
...
HRESULT CMonster_Lamp::Once_AnimMotion(_double TimeDelta) //무빙넘버와 공격넘버로 바꿔도 괜찮을듯?
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
