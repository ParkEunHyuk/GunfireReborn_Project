...
class CMonster_Lamp final : public CLandObject
{
private:
	explicit CMonster_Lamp(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CMonster_Lamp(const CMonster_Lamp& rhs);
...
private://유니버설 블렛을 위한 함수
	HRESULT	Universal_Wield(_double TimeDelta); //몸통을 휘둘러서 공격
...
public:
	static CMonster_Lamp* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
