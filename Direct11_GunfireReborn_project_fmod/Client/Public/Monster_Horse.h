...
class CMonster_Horse final : public CLandObject
{
private:
	explicit CMonster_Horse(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CMonster_Horse(const CMonster_Horse& rhs);
	virtual ~CMonster_Horse() = default;
...
private://유니버설 블렛을 위한 함수
	HRESULT	Universal_Wield(_double TimeDelta); //칼을 휘둘러서 공격
	HRESULT	Universal_StrikeDown(_double TimeDelta); //칼로 내려찍기
...
public:
	static CMonster_Horse* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
