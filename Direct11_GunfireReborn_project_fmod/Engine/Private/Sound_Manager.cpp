...
HRESULT CSound_Manager::Initialize_FMOD()
{
...
HRESULT CSound_Manager::Update_FMOD(_double DeltaTime)
{
...
int CSound_Manager::Channel_VolumeUp(CHANNELID eID, _float _vol)
{
...
int CSound_Manager::Channel_VolumeDown(CHANNELID eID, _float _vol)
{
	//잠시 주석처리함
	//m_VolumeArr[eID] = _vol;
	//if (m_VolumeArr[eID] > SOUND_MAX)
...
int CSound_Manager::Channel_Pause(CHANNELID eID)
{
...
HRESULT CSound_Manager::PlaySound(TCHAR * pSoundKey, CHANNELID eID, _float fLouderMultiple)
{
	if (eID == CHANNEL_BGM)
		return E_FAIL;
...
HRESULT CSound_Manager::PlayBGM(TCHAR * pSoundKey, _float fLouderMultiple)
{
	map<TCHAR*, FMOD_SOUND*>::iterator iter;
...
void CSound_Manager::Stop_ChannelSound(CHANNELID eID)
{
	if (eID == CHANNEL_BGM)
		FMOD_Channel_Stop(m_pChannelArr[31]);
	else
...
void CSound_Manager::Stop_AllChannel()
{
	for (_uint i = 0; i < 32; ++i) {
...
_float CSound_Manager::Get_Channel_Volume(CHANNELID eID)
{
	return m_VolumeArr[eID];
}

_bool CSound_Manager::Get_Channel_IsPaused(CHANNELID eID)
{
