...
HRESULT CModel::NativeConstruct(void * pArg)
{
	...
	m_AnimBlending = CAnimBlending::Create();
...
HRESULT CModel::Update_CombinedTransformationMatrices(_double TimeDelta, _uint _iCurrentAnim,_uint _iNextAnim, _bool& _pSwitch)
{
	/* 모든 뼈들의 현재 시간에 맞는 상태를 저장시킨다.(채널에)  */

	if (true == _pSwitch)
	{
		_pSwitch = m_Animations[_iCurrentAnim]->Update_Once(TimeDelta);
	}
	else {
		m_Animations[_iNextAnim]->Update_TransformMatrices(TimeDelta);
	}
...
HRESULT CModel::Render(CShader * pShader, _uint iPassIndex, _uint iMaterialIndex, const char* pBoneValueName)
{
	...
	for (auto& pMeshContainer : m_pMeshContainers[iMaterialIndex])
	{
		if (TYPE_ANIM == m_eModelType)
		{
			...
			pShader->Set_RawValue(pBoneValueName, BoneMatrices, sizeof(_float4x4) * 256);
		}
...
HRESULT CModel::SetUp_AnimIndex(_uint iAnimIndex)
{
	...
	//애니메이션 블랜딩이 진행되는 도중에 또 같은 애니매이션으로 바꾸려고 하면  제대로 변경이 안되니까 예외처리용
	if (m_iNextAnim == iAnimIndex)
		return S_OK;
	
	m_AnimBlending->Set_Blending(m_Animations[m_iCurrentAnim], m_Animations[iAnimIndex]);//추가됨
	m_iNextAnim = iAnimIndex;
	
	m_Animations[m_iCurrentAnim]->Reset();
	
	return S_OK;
}

HRESULT CModel::Update_CombinedTransformationMatrices(_double TimeDelta,_bool& _bOneTimeSwith)
{
	if (false == m_AnimBlending->Get_Blending_Finished()) //블렌더 용
	{
		if (true == m_AnimBlending->Update_TranformMatrices(TimeDelta))
		{
			m_iCurrentAnim = m_iNextAnim;
		}
	}
	else
	{
		m_Animations[m_iCurrentAnim]->Update_TransformMatrices(TimeDelta, _bOneTimeSwith);
	}
