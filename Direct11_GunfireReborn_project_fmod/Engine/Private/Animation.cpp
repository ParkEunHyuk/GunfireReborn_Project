...
HRESULT CAnimation::Update_TransformMatrices(_double TimeDelta)
{
	...
	for (_uint i = 0; i < m_iNumChannels; ++i)
	{
		...
		m_Channels[i]->Set_SaveFrame(vScale, vRotation, vPosition); // Save 부분
...
HRESULT CAnimation::Update_TransformMatrices(_double TimeDelta, _bool & _bOneTimeSwith)
{
	/* 현재 내 애니메이션의 재생 위치. */
	m_PlayTimeAcc += m_TickPerSecond * TimeDelta;
...
_bool CAnimation::Update_Once(_double TimeDelta)
{
	/* 현재 내 애니메이션의 재생 위치. */
	m_PlayTimeAcc += m_TickPerSecond * TimeDelta;
